...
function Item({ imgUrl, title, price }: { imgUrl: string, title: string, price: Number }) {
    ...
    return (
        <div className="flex flex-col justify-center h-[500px] w-[350px] shadow-lg border-2 border-blue-700 rounded-lg">
            ...
            <div className="flex p-2 justify-center flex-col gap-1 h-[35%]">
                ...
                <button className="max-w-[7rem] border-1 p-2 bg-yellow-400 text-black rounded-lg">Add to cart</button>
            </div>
        </div>
    )
}
        ...
        <div className="flex flex-wrap gap-10 items-center justify-center h-fit p-10 mb-10">
            ...
            }
            {
                data && data.map((item: any) => {
                         return <Item key={item.id} imgUrl={item.imgUrl} title={item.title} price={item.price}></Item>
                })
            }
        </div>
    )
